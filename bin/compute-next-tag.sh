...
#
# Tags look like `v<node exporter version>-<release>`:
#
# - if defaults/main.yml points at a node exporter version that has never been
#   released, the release counter restarts at 0 (`v1.12.1-0`)
# - otherwise the counter is incremented (`v1.12.1-1`), but only if something
...
# (`prometheus_node_exporter_container_image_tag`, for one) can be picked up
# instead.
version="$(sed -nE 's|^prometheus_node_exporter_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the node exporter version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v1.12.1`),
